...
rf_weighted <- train(y ~ .,
                     ...
                     method = "rf",
                     weights = model_weights,
                     tuneGrid =  expand.grid(mtry = seq(floor(ncol(train) / 4), floor(ncol(train) / 2), length.out = 50)),
                     ntree = 500,
                     nodesize = 5,
                     metric = "ROC",
                     verbose = FALSE,
...
sgb_tuning = function(data){
  ...
  grid = expand.grid(shrinkage = seq(.01, .1, by = 0.001), interaction.depth = c(3, 5, 7, 9, 11, 13), n.trees = c(80, 100, 120, 150, 200), n.minobsinnode = 10)
