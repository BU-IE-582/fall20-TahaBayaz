...
knitr::opts_chunk$set(echo = TRUE, fig.align = "center", message = FALSE, warning = FALSE, error = FALSE, fig.width = 15, fig.height = 15)
```
...
```{r}
train_data = fread("IE582_Fall20_ProjectTrain.csv")
test_data = fread("IE582_Fall20_ProjectTest.csv")
head(train_data)
str(train_data)
```
...
ggplot(train_data, aes(x = y)) + geom_bar(aes(fill = y)) + scale_fill_brewer(palette = "Palette1")
```
...
ggplot(stack(train_data[, c("x1", "x2", "x3", "x4", "x5", "x6")]), aes(x = ind, y = values)) +
  geom_boxplot(aes(fill = ind)) + scale_fill_brewer(palette = "Palette1")
...
rf_weighted <- train(y ~ .,
                     ...
                     method = "rf",
                     weights = model_weights,
                     tuneGrid =  expand.grid(mtry = seq(floor(ncol(train) / 4), floor(ncol(train) / 2), length.out = 10)),
                     ntree = 100,
                     nodesize = 5,
                     metric = "ROC",
                     verbose = FALSE,
...
sgb_tuning = function(data){
  ...
  grid = expand.grid(shrinkage = seq(.01, .1, by = 0.05), interaction.depth = c(3, 5, 7, 9), n.trees = c(80, 100, 120), n.minobsinnode = 10)
...
```{r}
head(scores[order(scores$OverAll, decreasing = TRUE),], 5)
```
