...
```{r box plot}
All_MAPE = data.table(MAPE = c(MAPE_lag168, MAPE_lag48, MAPE_lr, model_lr_hourly$MAPE, model_lasso_min_hourly$MAPE, model_lasso_1se_hourly$MAPE), Model = c('lag 168 naïve approach', 'lag 48 naïve approach', 'LR', paste(0:23, 'Hour_LR', sep = '_'), paste(0:23, 'Hour_Lasso_Min', sep = '_'), paste(0:23, 'Hour_Lasso_1se', sep = '_')))
ggplot(aes(y = MAPE), data = All_MAPE) +
  geom_boxplot()
```

When we look at the results of all MAPE values, we see that almost half of the MAPE values are between 2 and 4. Most of these values are around 3.5. So, we can say that, if a model whose MAPE value is more than 4, it can be thought as an inefficient model respect to others. 

```{r MAPE}
head(setorder(All_MAPE, MAPE))
```

When we look at the top MAPE values, they belong to  `r setorder(All_MAPE, MAPE)[1,2]`. At the end of the task, we can come up with some information like below:
