...
Following functions are created in order to accomplish the objective. First function, *prepare_data*, is used to import the data and rename the columns. Second function, *transform_long*, is used to transform the wide data into long data, which is called melting the data. Third function, *coord*, is used to calculate the coordinate information from accelerometer information, which taking the cumulative summation of the cumulative summation of accelerometer. Fourth function, *pca_plot*, is used to plot the first component of the PCA result. In the plot, we randomly pick 2 instances of the class. The fifth function, *pca_class*, is used to apply PCA to each class separately.
...
```{r}
pca_plot(4)
plot_ly(x = x_coord[,"X_4"], y = y_coord[,"Y_4"], z = z_coord[,"Z_4"], type="scatter3d", mode="markers")
```
...
```{r}
pca_plot(5)
plot_ly(x = x_coord[,"X_5"], y = y_coord[,"Y_5"], z = z_coord[,"Z_5"], type="scatter3d", mode="markers")
```
...
```{r}
pca_plot(7)
plot_ly(x = x_coord[,"X_7"], y = y_coord[,"Y_7"], z = z_coord[,"Z_7"], type="scatter3d", mode="markers")
```
...
```{r}
pca_plot(6)
plot_ly(x = x_coord[,"X_6"], y = y_coord[,"Y_6"], z = z_coord[,"Z_6"], type="scatter3d", mode="markers")
```
...
```{r}
pca_plot(8)
plot_ly(x = x_coord[,"X_8"], y = y_coord[,"Y_8"], z = z_coord[,"Z_8"], type="scatter3d", mode="markers")
```
